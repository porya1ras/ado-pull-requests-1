...
export class PrWebviewProvider implements vscode.WebviewViewProvider {
        ...
        webviewView.webview.onDidReceiveMessage(async data => {
            switch (data.type) {
                ...
                case 'openFileDiff':
                    ...
                    break;
                case 'aiReview':
                    await this.handleAiReview(data.action, data.prId, data.repoId, data.orgUrl);
                    break;
                case 'signIn':
    ...
    public async refresh() {
        if (this._view) {
            this._view.webview.postMessage({ type: 'loading', value: true, text: 'Refreshing PRs and Repos...' });
    ...
    private async handleFetchAllRepos() {
        ...
        this._view.webview.postMessage({ type: 'loading', value: true, text: 'Updating Repository List...' });
    ...
    private async handleSetRepo(repo: any) {
        ...
        vscode.window.showInformationMessage(`Active repository set to: ${repo.name}`);
        await this.updateState();
    }
    ...
    public async updateState() {
        ...
        try {
            ...
            const sourceBranches = Array.from(new Set(prs.map(pr => pr.sourceRefName?.replace(/^refs\/heads\//, '') || '?'))).sort();
            const targetBranches = Array.from(new Set(prs.map(pr => pr.targetRefName?.replace(/^refs\/heads\//, '') || '?'))).sort();
            ...
            if (err?.message === 'Not signed in') {
                ...
                this._view.webview.postMessage({ 
                    type: 'error', 
                    message: 'Failed to load Pull Requests. Please check your connection and try again.' 
                });
            }
        }
    }
    ...
    private async handlePrClick(prId: number, repoId: string, orgUrl: string) {
        ...
        this._view.webview.postMessage({ type: 'loading', value: true, text: 'Loading PR Details...' });
        ...
        try {
            ...
            if (!iterations || !iterations.length) {
                vscode.window.showInformationMessage(`No iterations found for PR #${prId}.`);
                return;
            }
            ...
            if (err?.message === 'Not signed in') {
                ...
                vscode.window.showErrorMessage(`Error fetching files: ${err.message || err}`);
            }
        } finally {
            this._view.webview.postMessage({ type: 'loading', value: false });
        }
    ...
    private async handleAiReview(action: string, prId: number, repoId: string, orgUrl: string) {
        const node = {
            kind: 'pr',
        ...
        let commandId = '';
        switch (action) {
            case 'copilot': commandId = 'adoPr.copilotReview'; break;
            case 'db': commandId = 'adoPr.dbPerformanceReview'; break;
            case 'ux': commandId = 'adoPr.uxMessageReview'; break;
        }
